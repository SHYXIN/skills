...
1. **先翻译，再动手**。每次拿到 issue / PR / 仓库描述原文，先用 2-3 句大白话告诉用户：
   - 这是个什么项目 / 什么任务
   - 报告人到底想要什么
   - 做完了长什么样
   
   不要直接把原始英文描述或 issue body 丢给用户。若用户明显不熟悉 issue / PR / fork 等概念，顺带用一句话类比解释（例如：「issue 就像在公共笔记本上贴一张便利贴提需求；PR 是你写好代码请求合并」）。

2. **过程实时播报**。每个非闲鱼动作（搜索仓库、拉仓库信息、打分）之前，用一句大白话说明「接下来做什么、为什么」；动作之后用一句说明「刚发生了什么、有什么变化（如：找到 N 个候选）」。让用户始终跟得上进度，不用反复问。

3. **主动给状态小结 + 下一步**。每完成一步，给一段极简总结：「当前状态」+「下一步是什么 / 在等什么（你 / 我）」。需要用户拍板时，明确列出选项并给出**你的推荐**，不要留白等用户来问。本技能只做发现与评估，真正接 issue 交给 oss-triage，切换前把「接下来走 oss-triage 挑哪个仓库」讲清楚。
...
4. 逐个体检，每个仓库检查：
   ...
   ```
   对每个候选，**先给用户一句大白话翻译它在做什么 / 解决什么问题**，再列打分。
5. 打分并排序。评分维度与权重如下。
...
- 结尾必须给一句「下一步是什么 / 在等什么（你 / 我）」，不要留白让用户来问。
