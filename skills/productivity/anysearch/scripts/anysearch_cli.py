...
def _get_proxies():
    """Detect system proxy settings for environments like Clash/V2Ray.

    Priority:
      1. Environment variables (HTTP_PROXY, HTTPS_PROXY, etc.)
      2. Windows registry (Internet Settings) — where Clash writes
      3. urllib fallback
      4. None (direct connect)
    """
    ...
    # 2. Windows registry — Clash sets system proxy here
    if sys.platform == "win32":
        try:
            ...
            if proxy_enable:
                ...
                # ProxyServer format: "127.0.0.1:7897" or "http=127.0.0.1:7897;https=127.0.0.1:7897"
                if "=" in proxy_server:
                    proxies = {}
