...
2. **过程实时播报**。每个非闲鱼动作（拉 issue、克隆、跑测试、commit、push、开 PR、留言）之前，用一句大白话说明「接下来做什么、为什么」；动作之后用一句说明「刚发生了什么、有什么变化（如：分支已建 / commit 已加 / PR 已开）」。让用户始终跟得上进度，不用反复问。

3. **主动给状态小结 + 下一步**。每完成一步，给一段极简总结：「当前状态」+「下一步是什么 / 在等什么（你 / maintainer / CI）」。需要用户拍板时，明确列出选项并给出**你的推荐**，不要留白等用户来问。涉及 GitHub 共享状态变更（fork / push / 开 PR / 留言 / force push）仍须先征得确认，但确认时要把「要做什么、影响是什么」讲清楚。
...
2. 确认三件事，缺一不可：
   ...
   拿到 issue 后，**先用大白话翻译一遍**（见上方「沟通准则」第 1 条：这是什么 / 报告人想要什么 / 做完什么样），再确认方案是否匹配。
3. 读上游贡献约定：
   ```bash
