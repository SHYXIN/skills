...
- **跨组件对比性引用（不自包含）**：组件描述自身规则时用「与 X 一致」「同 X 的体验」「对齐 X」（X 为同仓库/同套配置里的其它组件）式对比——读者须跳到 X 才能理解本组件规则。建议改写为自包含陈述（把规则本身写全）；例外：若本组件职责就是编排/路由，指向其它组件的指令性提及属运行时需要，不按此条处理。
...
```markdown
## 流程
本组件的登录流程与 foo-auth 一致，查询体验同 foo-auth，错误处理对齐 foo-auth。
```
> 三处规则全靠跳转 foo-auth 才能读懂。改写为自包含陈述：把登录、查询、错误处理的规则本身写全（即使与 foo-auth 实际相同）。

**编排层正例（应留）**：

```markdown
## 路由
涉及鉴权时调用 foo-auth 组件；本组件不自行实现登录。
```
> 编排/路由组件的职责就是指向其它组件，指令性提及是运行时需要，保留。
