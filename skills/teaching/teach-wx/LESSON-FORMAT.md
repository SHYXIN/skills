...
- 中文优先，专业名词保留英文或缩写。
- 使用简单、可打印的共享样式。
- 字符图放进 `<pre><code>`，按 `../DIAGRAM-STYLE.md` 规范画（盒线字符、宽度对齐）。
- 不写官方文档腔，也不过度口语化。
- 不把旁路问答放进 lesson。
...
- 实操型主题 lesson 默认包含“示例项目”章节，链接到 `../examples/000N-{slug}/`；示例项目章节必须写清：目标、路径、运行命令、预期结果、观察重点。
- 概念型主题 lesson 用“架构观察”章节替代示例项目：按 DIAGRAM-STYLE 画核心架构图/对比矩阵，附伪代码和“值得去读的真实文件/资料清单”（如仓库关键模块路径）。不生成 examples/。
- 环境跑不了示例时（缺运行时、需要 Linux 特性等），即使实操型也降级：换本机可跑的等价示例，或改为标注环境的示意代码 + 预期输出并注明“未验证”。
...
<body>
  <article class="lesson">
    ...
    <section id="demo">
      ...
      <!-- 概念型：架构观察（替代上面整个 section 内容） -->
      <!--
      <h2>架构观察</h2>
      <pre><code>{按 DIAGRAM-STYLE 画的核心架构图或对比矩阵}</code></pre>
      <p class="caption">{2-4 句话解释图中箭头和边界}</p>
      <ul>
        <li>值得去读：<a href="{资料或仓库文件路径}"><code>{真实文件/资料}</code></a> — {读什么、看哪个机制}</li>
      </ul>
      -->
    </section>
