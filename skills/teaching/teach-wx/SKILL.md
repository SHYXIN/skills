...
5. **结构优先。** 能画流程、层级、关系、数据流时，优先用字符图。规范见「字符图规则」。图后必须解释箭头含义。
...
- `./examples/*`：技术 lesson 配套的小示例和共享运行环境。格式见 [EXAMPLE-FORMAT.md](./EXAMPLE-FORMAT.md)。**只给实操型主题**默认生成（见下方「示例与架构观察」）；概念型主题默认不生成。
- `./diagrams/*`：概念型主题的架构观察图源文件（可选）。图直接嵌 lesson 时可不单独建目录。
...
4. 如果这是新主题，先初始化学习区：创建 `MISSION.md`、`OUTLINE.md`、`RESOURCES.md`、`GLOSSARY.md`、`NOTES.md`、`lessons/`、`learning-records/`、`side-questions/`、`assets/`。`examples/` 只在判定为实操型时创建。缺什么补什么，不覆盖已有内容。
5. 如果 `MISSION.md` 不存在或目标很模糊，先问清楚为什么学，再写入 mission。写入时判定并记录「主题性质：概念型 / 实操型」（见下方「示例与架构观察」）。
6. 如果没有 `OUTLINE.md`，先给 2-4 个可完成学习单元作为大纲，并记录当前位置。
7. 事实问题优先查可信资料或代码库，不要凭记忆硬讲。需要外部资料时，使用高可信来源，并按需写入 `RESOURCES.md`。

## 示例与架构观察

可运行示例的价值取决于「跑一遍能否加深理解」，不是所有主题都需要代码。

### 主题性质判定

初始化 `MISSION.md` 时判定并记录一行：

```text
## 主题性质
概念型（偏原理 / 架构 / 机制理解，跑代码收益低）或实操型（要上手用 API / 工具 / 库）。
```

- 模型自动判断，用户看到不对可以纠正；纠正后更新 mission。
- **仓库学习不豁免**：学仓库但目标偏架构评估、源码导览时，照样可判为概念型。判「实操型」的依据是 mission 里明确要「学会用」「跑通」「集成」。
...
|---|---|---|
| lesson 章节 | 示例项目 | 架构观察 |
| 产物 | `examples/000N-{slug}/` 可运行示例 | ASCII 架构图、伪代码、值得读的真实文件/资料清单 |
| 默认 | 生成 | 不生成 examples/ |

### 环境可行性否决

即使判为实操型，如果本机跑不了（缺运行时、需要 Linux 特性如 cgroup / unshare 而当前是 Windows 等），降级处理：

1. 优先换一个本机可跑的最小等价示例；
2. 没有等价物时，改为标注运行环境的示意代码 + 预期输出，明确写「未验证」和原因；
3. 不为跑示例而要求用户先装环境，除非 mission 目标就是学会那个环境。
...
5. 按 MISSION.md 里的主题性质决定产物：实操型为技术 lesson 规划一个 5-10 分钟可运行的小示例，放入 `examples/000N-{slug}/`，依赖使用 `examples/` 级别共享环境；概念型不出示例，lesson 用「架构观察」章节（图 + 值得读的文件清单）。
...
8. **示例或架构观察**：实操型主题默认生成可运行小示例并在 lesson 中链接；概念型主题的 lesson 用「架构观察」章节（按 DIAGRAM-STYLE 画图 + 值得读的真实文件/资料清单），不生成 examples/。
9. **完成检查**：按 [LESSON-CHECKLIST.md](./LESSON-CHECKLIST.md) 检查 HTML、示例/架构观察、学习区状态和对话收尾。
...
6. 最小例子
7. 常见误解和边界
8. 示例项目（实操型）或架构观察（概念型）
9. 自检问题
10. 下一步

页面采用 Tufte-ish 技术讲义风格：白底、窄正文栏、充足留白、清晰排版层级、克制颜色、适合打印和长期复习。不要做 dashboard/card UI，不要使用大面积彩色块，不要让视觉样式喧宾夺主。每节 lesson 默认包含一个短的“直觉类比”，但类比只负责建立直觉，不代替定义；必须说明它哪里像、哪里不像，并回到技术结构。不要写成长故事。实操型 lesson 默认包含“示例项目”章节，链接到 `../examples/000N-{slug}/`，写清运行命令和观察重点；概念型 lesson 用“架构观察”章节替代：按 [DIAGRAM-STYLE.md](./DIAGRAM-STYLE.md) 画核心架构图/对比矩阵，附伪代码和「值得去读的真实文件/资料清单」（如仓库关键模块路径）。每节 lesson 应包含顶部元信息、页内目录、稳定锚点、克制的一句话结论、自检题块，以及底部“回到大纲 / 上一课 / 下一课 / 参考资料”链接；不存在的链接直接省略。

## 字符图规则

图的质量取决于选型和对齐，按 [DIAGRAM-STYLE.md](./DIAGRAM-STYLE.md) 的规范画。该规范吸收自 tui-diagram 技能：盒线字符（┌─┐│└┘）、中英文按显示宽度对齐（中文占 2 列）、总宽 ≤80 列、箭头用 → ▼ ▶。

要点：

1. **按语义选图型**：流程用流程图、组成用架构图、调用链用时序图、方案对比用对比矩阵、层级用树。不确定时看用户在问“怎么做”还是“由什么组成”还是“哪个好”。
2. **对齐自查**：画完检查同一列上的 `│` 是否纵向对齐，中英文混排处宽度是否算对。
3. **禁止 ASCII 混排**：不要用 `+--+`、`->`、`|` 画线，统一用盒线字符和 ▶▼。
4. 图下面必须用 2-4 句话解释箭头的含义。不要只放图不解释。
...
- `MISSION.md`：新主题开始时写入（含主题性质判定）；学习目标或性质判定变化时更新。
- `OUTLINE.md`：新主题开始、主线变化、当前位置变化时写入。
- `REPO-BRIEF.md`：仓库学习默认先写导览，后续 lesson 以它为地图。
- `lessons/*.html`：主线学习单元默认写入或更新。
- `examples/000N-{slug}/`：仅实操型主题默认写入一个可运行小示例；依赖共享在 `examples/` 级别。概念型主题不生成。
...
- 不要给概念型主题硬造可运行示例；也不要给实操型主题只讲图不给代码。
